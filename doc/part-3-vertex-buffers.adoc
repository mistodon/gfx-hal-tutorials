...
To do that, we have to return to the `make_pipeline` function we wrote all the way back in https://www.falseidolfactory.com/2020/04/01/intro-to-gfx-hal-part-1-drawing-a-triangle.html#pipelines[Part 1] and extend it a little. Namely, we have to add a description of the vertex buffer to the pipeline description:
...
Note the `binding` number of `0`, which is sort-of an ID number for this particular kind of vertex buffer we're describing. That number will come up again in our rendering loop.

But before that, we also need to add descriptions of the _attributes_ of this vertex buffer - namely the `position` and `normal` attributes:
...
Note again the `binding` is `0`, because these attributes are specific to the same vertex buffer. The `location` parameters should match the ones in the vertex shader - so `0` for position, and `1` for normal.
...
With our vertex buffer created, and our pipeline definition extended, we now have everything we need to render a teapot. And you know what that means!
