...
fn main() {
    ...
    unsafe fn make_pipeline<B: gfx_hal::Backend>(
        ...
        // Vertex buffer description
        {
            ...
            // We need to add a new section to our pipeline description so that
            // it understands how to interpret the vertex data it is given.
            //
            // We start by giving it a `binding` number, which is more or less
            // an ID or a slot for the vertex buffer. You'll see it used later.
            //
            // The `stride` is the size of one item in the buffer. The `rate`
            // defines how to progress through the buffer. Passing `Vertex` to
            // this tells it to advance after every vertex. This is usually
            // what you want to do if you're not making use of instanced
            // rendering.
            ...
            // Now we need to define the attributes _within_ the vertices. For
            // us this is the `position` and the `normal`.
            // The vertex buffer we just defined has a `binding` number of `0`.
            // The `location` refers to the location in the `layout` definition
            ...
            // vectors, and so the `format` is `Rgb32Sfloat`. (I don't know why
            // it's `Rgb` and not `Xyz` or `Vec3` but here we are.)
            //
